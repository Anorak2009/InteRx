...
def main():
    ...
    with tab2:
        if ror_df.empty:
            ...
            if only_dangerous and "is_dangerous" in filtered.columns:
                filtered = filtered[filtered["is_dangerous"]].copy()
    ...
    with tab4:
        ...
        if ror_df.empty:
            ...
            if only_dangerous and "is_dangerous" in summary_df.columns:
                summary_df = summary_df[summary_df["is_dangerous"]].copy()
            if only_novel and known_interactions:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # TAB 6 — Name Lookup (RxNorm)
    # ─────────────────────────────────────────────────────────────────────────
    with tab6:
        st.subheader("Drug Name Lookup")
        st.caption(
            "Convert commercial/brand names to official FDA generic names using the "
            "NIH RxNorm database. Results are fetched live — no pipeline run required."
        )
        ...
        do_search = col_search.button("Look up", type="primary")
        ...
        if st.session_state.rxnorm_results:
            ...
            if candidates and "error" in candidates[0]:
                ...
                st.warning(
                    ...
                    "Try an alternate spelling or the generic name."
                )
            else:
                # Separate brand hits from generic hits
                ...
                other_hits   = [c for c in candidates if c not in brand_hits and c not in generic_hits]

                st.success(f"Found **{len(candidates)}** result(s) for **{query}**")

                # ── Primary result card ───────────────────────────────────────
                # Pick the best hit: prefer exact brand match → generic → first result
                best = next(
                ...
                with st.container(border=True):
                    ...
                    # Resolve ingredients for brand drugs
                    if is_brand and best["rxcui"]:
                        ...
                        if ingredients and "error" not in ingredients[0]:
                            ...
                            # For each ingredient, also fetch any other brand names
                            all_brands = []
                            for ing in ingredients:
                                all_brands.extend(rxnorm_get_brand_names(ing["rxcui"]))
                            # Deduplicate and exclude the query itself
                            other_brands = sorted(set(b for b in all_brands if b.lower() != query.lower()))

                            if other_brands:
                                st.markdown(
                                    "**Other brand names for this ingredient:** "
                        ...
                        # It's already a generic — show brand names
                        ...
                        if brands:
                            c3.metric(
                                ...
                                help="Brand names that contain this generic ingredient.",
                            )
                            if len(brands) > 1:
                                st.markdown(
                                    "**All brand names:** "
                ...
                if is_brand and best["rxcui"]:
                    ingredients = rxnorm_get_ingredients(best["rxcui"])
                    if ingredients and "error" not in ingredients[0]:
                        generic_name = ingredients[0]["name"].lower()
                        matched_in_db = generic_name in [d.lower() for d in all_drugs]
                        if not matched_in_db:
                            st.info(
                                f"Generic name **{generic_name}** is not in the current "
                                "FAERS drug index. It may appear under a different spelling — "
                                "try searching for it in the Drug Pair Lookup tab directly."
                            )
...
def rxnorm_search(name: str) -> list[dict]:
    """
    Search RxNorm for a drug name (brand or generic).
    Returns a list of candidate dicts with keys: rxcui, name, tty, synonym.
    """
    import requests
    try:
        resp = requests.get(
...
def rxnorm_get_ingredients(rxcui: str) -> list[dict]:
    """
    Given an RxCUI (brand or SBD), resolve to active ingredient(s).

    Strategy (in order):
      1. Try /related?tty=IN  (most reliable for BN concepts)
      2. Try /related?tty=MIN (multiple ingredients)
      3. If the concept is an SBD, walk up to its BN parent first via
         /related?tty=BN, then resolve IN from there
      4. Fall back to /rxcui/{rxcui}/allrelated.json and pull IN/MIN rows
    Returns list of dicts with rxcui + name of each ingredient, or [{"error": ...}].
    """
    import requests

    def _fetch_related(cui: str, tty: str) -> list[dict]:
        resp = requests.get(
            f"{RXNORM_BASE}/rxcui/{cui}/related.json",
            params={"tty": tty},
            timeout=10,
        )
        resp.raise_for_status()
    ...
    try:
        # Strategy 1 — IN directly
        found = _fetch_related(rxcui, "IN")
        if found:
            return found

        # Strategy 2 — MIN (multi-ingredient generics)
        found = _fetch_related(rxcui, "MIN")
        if found:
            return found

        # Strategy 3 — walk SBD → BN parent, then resolve IN from BN
        bn_parents = _fetch_related(rxcui, "BN")
        for parent in bn_parents:
            found = _fetch_related(parent["rxcui"], "IN")
            if found:
                return found
            found = _fetch_related(parent["rxcui"], "MIN")
            if found:
                return found

        # Strategy 4 — allrelated fallback, pick first IN or MIN rows
        resp = requests.get(
        ...
        return []  # nothing found — caller shows "Could not resolve"
...
def rxnorm_get_brand_names(rxcui: str) -> list[str]:
    """
    Given a generic ingredient RxCUI, return known brand names.
    """
    import requests
    try:
        resp = requests.get(
...
def rxnorm_get_fda_nda(rxcui: str) -> list[dict]:
    """
    Retrieve FDA NDA/ANDA application numbers linked to this RxCUI via RxNorm.
    """
    import requests
    try:
        resp = requests.get(
        ...
        return [{"type": p.get("propName"), "value": p.get("propValue")} for p in props]
    except Exception:
        return []
